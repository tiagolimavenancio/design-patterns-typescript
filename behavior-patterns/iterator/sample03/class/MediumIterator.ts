...
export class MediumIterator implements IProfileIterator {
  ...
  private lazyLoad(): void {
    if (this.emails.length === 0) {
      const profiles = this.medium.requestRelatedContactsFromMediumInAPI(this.email, this.type);
  ...
  public getNext(): Profile {
    ...
    if (!friendContact) {
      friendContact = this.medium.requestContactInfoFromMediumInAPI(friendEmail);
